...
import json
from .default import get_cfg_defaults, RENAME_KEYS
import os
...
def setup_cfg(cfg_file=[], set_cfgs=None, default: CfgNode=None, rename_dict=RENAME_KEYS) -> CfgNode:
    """
    ...
    for i in range(L//2):
        ...
        if rename_dict is not None and k in rename_dict:
            k = rename_dict[k]
    ...
    logdir = os.path.join(cfg.dataset, cfg.split,
                                    cfg.aux.exp, str(cfg.aux.runid))
    logdir = logdir.replace('-', '_') 
    # logdir_prefix = "/tmp/zijia-2024/logp2" if not cfg.aux.debug else "log_test/"
    logdir_prefix = "log" if not cfg.aux.debug else "log_test/"
    logdir = os.path.join(logdir_prefix, logdir)

    cfg.aux.logdir = logdir
    return cfg

def load_cfg_json(cfg_file, update_expname=False, update_cfg=False):
    ...
    if update_cfg:
        cfg = setup_cfg(cfg.aux.cfg_file, cfg.aux.set_cfgs)
